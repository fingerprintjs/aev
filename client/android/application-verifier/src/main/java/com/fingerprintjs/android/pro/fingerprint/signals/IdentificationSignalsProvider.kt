...
import com.fingerprintjs.android.fingerprint.signal_providers.Signal as OssSignal


interface IdentificationSignalsProvider {
    fun signals(
...
class IdentificationSignalProviderImpl : IdentificationSignalsProvider {
    override fun signals(
        fingerprintResult: FingerprintResult,
        deviceIdResult: DeviceIdResult
    ): List<Signal> {
        return fingerprintResult.getSignalProvider(HardwareSignalGroupProvider::class.java)
            ?.rawData()?.signals()?.map {
                object : Signal {
    ...
    private fun wrapSignalToMap(state: String, signal: OssSignal<*>): Map<String, Any> {
